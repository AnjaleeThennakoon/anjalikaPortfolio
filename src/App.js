...
// ── Asset imports (keep yours exactly as they were) ──────────────────────────
...
function use3DTilt(strength = 15) {
  ...
  const handleMove = useCallback((e) => {
    ...
    if (shine) { shine.style.background = `radial-gradient(circle at ${(x+0.5)*100}% ${(y+0.5)*100}%, rgba(255,255,255,0.18) 0%, transparent 70%)`; }
  }, [strength]);
...
export default function App() {
  ...
  // Personality fact trigger refs
  const fact0Ref = useRef(null);
  ...
  useEffect(() => {
    const tick = () => setTime(new Date().toLocaleTimeString("en-US",
      { hour:"2-digit", minute:"2-digit", second:"2-digit", hour12:false, timeZone:"Asia/Colombo" }));
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // personality popup triggers based on scroll position
  ...
  // Personality popups on scroll milestones
  useEffect(() => {
    const triggers = [
      { pct: 15, idx: 0 },
      { pct: 35, idx: 1 },
  ...
  return (
    <div className={`app${dark ? " dark" : ""}`}>
      {/* Custom cursor */}
      <div className={`cursor${curHover ? " cursor--big" : ""}`}
      ...
      <nav className={`nav${navScrolled ? " nav--scrolled" : ""}`}>
        <button className="nav-logo" onClick={() => scrollTo("hero")} onMouseEnter={on} onMouseLeave={off}>
        ...
        <div className={`nav-links${navOpen ? " nav-links--open" : ""}`}>
          {navLinks.map(([id, label]) => (
            <button key={id}
                    ...
                    onClick={() => scrollTo(id)} onMouseEnter={on} onMouseLeave={off}>
              {label}
            </button>
          ))}
          <a href={anjaleeCV} download className="nav-cv" onMouseEnter={on} onMouseLeave={off}>
            CV ↓
          </a>
        </div>
        ...
        <div className="nav-right">
          ...
          <button className={`burger${navOpen ? " burger--open" : ""}`}
      ...
      <section id="hero" className="hero" ref={heroRef}>
        ...
        <div className={`hero-content${heroIn ? " hero-content--in" : ""}`}>
          <div className="hero-profile-wrap">
            <div className="orbit or-1" /><div className="orbit or-2" /><div className="orbit or-3" />
          ...
          <div className="hero-text">
            ...
            <div className="role-tags">
              {["Full-Stack Developer","UI/UX Designer","IoT Enthusiast"].map((r,i)=>(
                <span key={r} className="role-tag" style={{ animationDelay:`${i*0.15+0.4}s` }}>{r}</span>
              ))}
            </div>
            <p className="hero-bio">
              Building scalable web & mobile applications with <strong>Java</strong>, <strong>C#</strong>,{" "}
              <strong>ASP.NET</strong>, <strong>Flutter</strong> and exploring the intersection of
              design and embedded systems with <strong>Arduino</strong>.
            </p>
            ...
            <div className="quick-stats">
              {[{icon:"🎯",num:"8+",label:"Projects"},{icon:"🏆",num:"7+",label:"Certs"},
                {icon:"🤝",num:"4",label:"Orgs"},{icon:"🎓",num:"2027",label:"Graduating"}
              ].map((s,i) => (
                <TiltCard key={s.label} className="stat-card" style={{animationDelay:`${i*0.1+0.6}s`}}>
      ...
      <div className="marquee" aria-hidden="true">
        <div className="mtrack">
          {[0,1,2].map(i => <span key={i} className="mitem">{MARQUEE_TEXT}</span>)}
        </div>
      </div>
      ...
      <section id="about" className="section about" ref={aboutRef}>
        ...
        <div className="about-grid">
          <div className={`about-story${aboutIn ? " reveal" : ""}`} style={{transitionDelay:"0.1s"}}>
            <TiltCard className="story-card">
              <div className="story-icon">🚀</div>
              <h3>My Journey</h3>
              <p>I'm a <strong>Software Engineering undergraduate</strong> at NSBM Green University,
              Homagama, Sri Lanka. My passion lies in creating robust, user-centered applications
              that solve real-world problems through elegant code and thoughtful design.</p>
              <p>From <strong>full-stack web platforms</strong> with ASP.NET Core and React, to{" "}
              <strong>intuitive mobile experiences</strong> with Flutter, and even tinkering with{" "}
              <strong>Arduino-powered robotics</strong> — I love the entire spectrum.</p>
            </TiltCard>

            <TiltCard className="story-card" style={{transitionDelay:"0.2s"}}>
              <div className="story-icon">💡</div>
              <h3>What Drives Me</h3>
              <p>I believe great software is born at the intersection of <strong>technical excellence</strong>{" "}
              and <strong>human-centered design</strong>. Every project gets curiosity and
              meticulous attention to detail — whether it's a scalable backend, pixel-perfect UI,
              or an IoT prototype.</p>
            </TiltCard>
          </div>

          <div className={`about-details${aboutIn ? " reveal" : ""}`} style={{transitionDelay:"0.25s"}}>
            <TiltCard className="details-card">
              <h4 className="details-heading"><span>👥</span>Leadership & Community</h4>
            ...
            <TiltCard className="details-card" style={{marginTop:"16px"}}>
              <h4 className="details-heading"><span>📍</span>Location & Education</h4>
              <div className="info-pills">
                {[
                  ...
                  <div key={p.label} className="info-pill">
                    <span>{p.icon}</span>
                    <div><p className="pill-label">{p.label}</p><p className="pill-value">{p.value}</p></div>
                  </div>
                ))}
              </div>
            </TiltCard>
            ...
            <div className="social-links">
              {[
                ...
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                   className="social-link" onMouseEnter={on} onMouseLeave={off}>
      ...
      <section className="facts-row" ref={factsRef}>
        <div className="facts-inner">
          {PERSONALITY_FACTS.map((f, i) => (
            <div key={i} className={`fact-chip${factsIn ? " reveal" : ""}`}
          ...
          {skillGroups.map((g, gi) => (
            <TiltCard key={g.label}
              ...
              <div className="skill-pills">
                {g.items.map((item, ii) => (
                  <span key={item} className="skill-pill" style={{ animationDelay: `${gi*0.1+ii*0.06}s` }}>{item}</span>
                ))}
              </div>
            </TiltCard>
          ))}
            ...
            <TiltCard key={p.id}
              ...
              <div className="pc-img-wrap">
                ...
                <div className="pc-overlay">
                  {p.github && <a href={p.github} target="_blank" rel="noopener noreferrer" className="pc-btn" onMouseEnter={on} onMouseLeave={off}>⌥ GitHub</a>}
                  {p.figma  && <a href={p.figma}  target="_blank" rel="noopener noreferrer" className="pc-btn pc-btn--figma" onMouseEnter={on} onMouseLeave={off}>🎨 Figma</a>}
                </div>
      ...
      <section id="certs" className="section" ref={certsRef}>
        ...
        <div className="certs-scroll">
          {certs.map((c, i) => (
            <TiltCard key={i} className={`cert-card${certsIn ? " reveal" : ""}`}
      ...
      <section id="contact" className="contact" ref={contactRef}>
        <div className="contact-blob-1" /><div className="contact-blob-2" />
        <div className={`contact-inner${contactIn ? " reveal" : ""}`}>
          <span className="section-label" style={{color:"rgba(202,240,248,.65)"}}>Let's Work Together</span>
          <h2 className="contact-title">
            Have a project in mind?<br/>
            <span className="contact-title-out">Let's build it.</span>
          </h2>
          <div className="contact-links">
            <a href="tel:+94703223968" className="contact-link" onMouseEnter={on} onMouseLeave={off}>📞 +94 703 223 968</a>
            <a href="mailto:anjalikarukshani51@gmail.com" className="contact-link" onMouseEnter={on} onMouseLeave={off}>✉ anjalikarukshani51@gmail.com</a>
            <a href="mailto:tmarthennakoon@students.nsbm.ac.lk" className="contact-link" onMouseEnter={on} onMouseLeave={off}>✉ tmarthennakoon@students.nsbm.ac.lk</a>
          </div>
          <div className="contact-btns">
            <a href="mailto:anjalikarukshani51@gmail.com" className="btn btn-primary" onMouseEnter={on} onMouseLeave={off}>Hire Me ↗</a>
            <a href={anjaleeCV} download className="btn btn-white" onMouseEnter={on} onMouseLeave={off}>Download CV ↓</a>
          </div>
        </div>
      </section>
      ...
      <footer className="footer">
        ...
        <div className="footer-social">
          {[
            ...
            <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
               className="footer-icon" onMouseEnter={on} onMouseLeave={off}>
              {s.icon}<span>{s.label}</span>
            </a>
          ))}
        </div>
